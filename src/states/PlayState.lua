...
function PlayState:SwapTiles(t1, t2, noMatches)
    self.board:SwapTiles(t1, t2)
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            local totalvariety = 0
            for k, tile in pairs(match) do
                totalvariety = totalvariety + tile.variety
            end
            if totalvariety <= 3 then
                self.score = self.score + #match * 50
                self.timer = self.timer + 6
            else
                self.score = self.score + #match * 50 * (totalvariety / 2)
                self.timer = self.timer + 2 * totalvariety
            end
        end
        ...
        if not self.board:MatchesExist() then
            while not self.board:MatchesExist() do
